...
def get_lockout_state():
    ...
    response=supabase.table("admin_lockout").select("*").eq("id",1).execute()
    if  len(response.data)==0:
        supabase.table("admin_lockout").insert({"id":1,"failed_attempts":0}).execute()
...
def record_failed_attempt(state):
    ...
    supabase.table("admin_lockout").update(update_data).eq("id",1).execute()
    return new_count

def reset_lockout():
    """ログイン成功時に失敗回数をリセットする"""
    supabase.table("admin_lockout").update({
        "failed_attempts":0,
        "locked_until":None,
        "last_attempt_at":datetime.now(timezone.utc).isoformat(),
    }).eq("id",1).execute()
